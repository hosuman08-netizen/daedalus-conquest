...
function enemyForChapter(ch){ch=Math.max(1,ch|0);return{drone:2+Math.floor((ch-1)/2),marksman:ch>=4?1+Math.floor((ch-4)/7):0,guardian:ch>=9?1+Math.floor((ch-9)/10):0,bruiser:ch>=14?1+Math.floor((ch-14)/9):0,commander:ch>=25?1+Math.floor((ch-25)/20):0,titan:ch>=50?1+Math.floor((ch-50)/40):0};}
