...
- **[2026-08-13·Morpheus+Tank+QA+Oracle]** 하루에 "조용히 죽는" 결함 4종 적발. ①launchd가 **맥북 뚜껑 닫힌 DarkWake 6~12초 창**에 잡을 발사 → DNS 사망 → curl 빈값 → 옛 코드가 "토큰 죽음"으로 단정(상태코드를 한 번도 안 봄) → 1753건 살포 ②워커 ALLOWED에 activate 등 14종 누락 → sendBeacon은 응답을 안 읽어 거부가 100
- **[2026-08-13·Morpheus+Tank+QA+Oracle]** 하루에 "조용히 죽는" 결함 4종 적발. ①launchd가 **맥북 뚜껑 닫힌 DarkWake 6~12초 창**에 잡을 발사 → DNS 사망 → curl 빈값 → 옛 코드가 "토큰 죽음"으로 단정(상태코드를 한 번도 안 봄) → 1753건 살포 ②워커 ALLOWED에 activate 등 14종 누락 → sendBeacon은 응답을 안 읽어 거부가 100% 침묵 → 같은 사고 3회 반복 ③sw.js 괄호누락이 22개에 복제, `.catch`로 조용히 실패 ④Plaud 감시자 초안이 404를 "신규 0건 정상"으로 위장할 뻔. → 무기화: **"조용히 실패"가 공통 병리다.** 신규 자동화 3원칙 — ①실패를 카운트/기록한다 ②모르면 침묵한다(거짓경보 금지) ③빈응답≠실패확정, 상태코드를 실제로 읽어라. 경보는 하루 1회 스로틀. 알람 피로가 진짜 신호를 죽인다(SMOKE-FAIL 118건이 그렇게 무시됐다).
